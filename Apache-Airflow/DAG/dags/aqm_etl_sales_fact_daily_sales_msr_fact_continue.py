...
default_args = {
    ...
    "start_date": datetime.datetime(2021, 6, 14),
    'retries': 1,
    'retry_delay': datetime.timedelta(seconds=5)
}

with DAG(
    'prism_aqm_dwh_etl_continue',
        ...
        def execute(self, context):
            ...
            for day in days_list:
                with closing(mysql_source.get_conn()) as conn:
                    with closing(conn.cursor()) as cursor:
                        # cursor.execute('set global max_allowed_packet=67108864')
                        cursor.execute("select mtime, dpid, route_id, skid, `group`, family, date, datetime, sale, dprice, rprice, dcc_price, issue, `return`, memos, vmemos, tlp, cnc, vp, mvp, p, tcc, dcc, ecnc, gt, struc, semi_struc, streetk, mass_hrc, pop_hrc, prem_hrc, kaccounts, ogrocery, snb_cnc, pay_n_go, shop_n_browse, entertainment, outlets, apps_version, updated, visited from daily_sales_msr where date between '"+day+"' and '"+day+"'")
                        ...
                        if self.bulk_load:
                            with NamedTemporaryFile("wb") as tmpfile:
                                self.log.info("Selecting rows from MySql to local file %s...", tmpfile.name)
                                self.log.info("select mtime, dpid, route_id, skid, `group`, family, date, datetime, sale, dprice, rprice, dcc_price, issue, `return`, memos, vmemos, tlp, cnc, vp, mvp, p, tcc, dcc, ecnc, gt, struc, semi_struc, streetk, mass_hrc, pop_hrc, prem_hrc, kaccounts, ogrocery, snb_cnc, pay_n_go, shop_n_browse, entertainment, outlets, apps_version, updated, visited from daily_sales_msr where date between '"+day+"' and '"+day+"'")
    ...
    query_manager_table_to_sales_fact_table = QueryManagerTableToSalesFactTableMonthlyDataEtxractorMySqlOperator(
                        ...
                        starting_date = datetime.date(2021, 1, 27),
                        ending_date = datetime.date(2021, 6, 10),
                        # start_month = datetime.date(2020, 1, 1),
                        # end_month = datetime.date(2021, 1, 1),
    ...
    delete_rows_from_sales_fact_table = MySqlOperator(
                        task_id = 'delete_rows_from_sales_fact_table',
                        mysql_conn_id = "aqm_dwh_server",
                        sql = "delete from sales_fact where date between '2021-01-27' and '2021-01-27'",
    )
    delete_rows_from_daily_sales_msr_fact_table = MySqlOperator(
                        ...
                        # sql = "delete from daily_sales_msr_fact where date between '2021-01-27' and '2021-01-27'",
    )
    ...
    start_operator >> delete_rows_from_sales_fact_table

    delete_rows_from_sales_fact_table >> delete_rows_from_daily_sales_msr_fact_table

    delete_rows_from_daily_sales_msr_fact_table >> query_manager_table_to_sales_fact_table
