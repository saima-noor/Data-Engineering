...
with DAG(
    'prism_aqm_dwh_etl_hrms_total_data_extractor',
    ...
    class HrEmployeeRecordLogsTableToHrEmployeeRecordLogsDimensionableMySqlOperator(BaseOperator):
        """
        Moves data from MySQL to MySQL.
        ...
        def execute(self, context):
            ...
            for day in days_list:
                with closing(mysql_source.get_conn()) as conn:
                    with closing(conn.cursor()) as cursor:
                        # cursor.execute('set global max_allowed_packet=67108864')
                        cursor.execute("select * from designations where date(updated_at) between '"+day+"' and '"+day+"'")
                        # cursor.execute("select mtime, dpid, route_id, skid, group, family, date, datetime, sale, dprice, rprice, dcc_price, issue, return, memoss, vmemos, tlp, cnc, vp, mvp, p, tcc, dcc, ecnc, gt, struc, semi_struc, streetk, mass_hrc, pop_hrc, prem_hrc, kaccounts, ogrocery, snb_cnc, pay_n_go, shop_n_browse, entertainment, outlets, apps_version, updated, visited from daily_sales_msr where month(date) = month("+month+") and year(date) = year("+month+")")
                        self.log.info(day)
                        # cursor.execute(self.sql)
                        selected_columns = [d[0] for d in cursor.description]
            ...
            for day in days_list:
                ...
                try:
                    if self.bulk_load:
                        ...
                        self.log.info("Inserting rows into MySQL destination...")
                        self.log.info(self.mysql_destination_table)
                        mysql_destination.insert_rows(table=self.mysql_destination_table, rows=result, target_fields=selected_columns)
                    self.log.info("Inserted rows into MySQL destination %s", count)
                except (MySQLdb.Error, MySQLdb.Warning):  # pylint: disable=no-member
                    self.log.info(mysql_destination_conn_cursor._executed)
    ...
    class BatLocationsTableToBatLocationsDimensionTableMySqlOperator(BaseOperator):
        """
        Moves data from MySQL to MySQL.
        ...
        def execute(self, context):
            ...
            with closing(mysql_source.get_conn()) as conn:
                with closing(conn.cursor()) as cursor:
                    # cursor.execute('set global max_allowed_packet=67108864')
                    cursor.execute("select * from bat_locations")
                    # cursor.execute("select mtime, dpid, route_id, skid, group, family, date, datetime, sale, dprice, rprice, dcc_price, issue, return, memoss, vmemos, tlp, cnc, vp, mvp, p, tcc, dcc, ecnc, gt, struc, semi_struc, streetk, mass_hrc, pop_hrc, prem_hrc, kaccounts, ogrocery, snb_cnc, pay_n_go, shop_n_browse, entertainment, outlets, apps_version, updated, visited from daily_sales_msr where month(date) = month("+month+") and year(date) = year("+month+")")
                    # self.log.info(day)
                    # cursor.execute(self.sql)
                    selected_columns = [d[0] for d in cursor.description]
    ...
    #                     ending_date = datetime.date(2020, 12, 31),
    ...
    # )
    #
    # loading_hr_employee_record_logs_dimension_table = HrEmployeeRecordLogsTableToHrEmployeeRecordLogsDimensionableMySqlOperator(
    ...
    #                     ending_date = datetime.date(2021, 8, 22),
    #                     mysql_source_conn_id = "hrm_bat",
    ...
    #                     starting_date = datetime.date(2019, 1, 1),
    #                     ending_date = datetime.date(2019, 12, 31),
    #                     mysql_source_conn_id = "hrm_bat",
    ...
    # loading_bat_locations_dimension_table = BatLocationsTableToBatLocationsDimensionTableMySqlOperator(
    #                     task_id = 'loading_bat_locations_dimension_table',
    #                     mysql_source_conn_id = "hrm_bat",
    #                     mysql_destination_table = "bat_locations_dimension",
    #                     mysql_destination_conn_id = "pass_aqm_dwh_database",
    #                     mysql_destination_preoperator = sql_statements.CREATE_bat_locations_dimension_TABLE_IF_NOT_EXISTS,
    # )


    # run_qsub_uality_checks = SQLCheckOperatoSubr(
    #   Sub                 task_id = 'Run_dasubta_quality_checks',
    ...
    send_success_email = EmailOperator(
                ...
                subject='ETL process for PRISM TOTAL HRMS AQM DWH',
    ...
    # start_operator >> loading_hr_employee_record_logs_dimension_table
    start_operator >> sys_users_table_to_sys_users_fact_table
    # start_operator >> hrm_emp_attendance_table_to_hrm_emp_attendance_fact_table
    # start_operator >> loading_sub_designations_dimension_table
    # loading_designations_dimension_table >> loading_sub_designations_dimension_table
    # loading_sub_designations_dimension_table >> loading_bat_locations_dimension_table
    # loading_bat_locations_dimension_table >> loading_hr_anomaly_dimension_table
    ...
    # hrm_emp_attendance_table_to_hrm_emp_attendance_fact_table >> sys_users_table_to_sys_users_fact_table

    sys_users_table_to_sys_users_fact_table >> run_quality_checks

    # loading_sub_designations_dimension_table >> run_quality_checks
