...
with DAG(
    'prism_aqm_dwh_etl_sales_fact_continue',
    ...
    class QueryManagerTableToSalesFactTableDailyDataEtxractorMySqlOperator(BaseOperator):
        ...
        def execute(self, context):
            ...
            with closing(mysql_source.get_conn()) as conn:
                with closing(conn.cursor()) as cursor:
                    # cursor.execute('set global max_allowed_packet=67108864')
                    cursor.execute("select mtime, rtslug, dpid, route_id, rtlid, retailer_code, skid, volume, value, date, outlets, visited, channel, ioq, sales_time from query_manager_table where date between '"+self.date+"' and '"+self.date+"'")
                    # self.log.info(cursor.execute())
                    # cursor.execute(self.sql)
                    # print(cursor.description)
                    print(self.date)
    ...
    #                     starting_date = datetime.date(2021, 6, 2),
    ...
    query_manager_table_to_sales_fact_table = QueryManagerTableToSalesFactTableDailyDataEtxractorMySqlOperator(
                        task_id = 'query_manager_table_to_sales_fact_table',
                        date = datetime.datetime.strftime(datetime.date(2021, 7, 25), '%Y-%m-%d'),
                        mysql_source_conn_id = "prism_live_114",
                        mysql_destination_table = "sales_fact",
                        mysql_destination_conn_id = "aqm_dwh_server",
                        mysql_destination_preoperator = sql_statements.CREATE_sales_fact_TABLE_IF_NOT_EXISTS,
    ...
    delete_rows_from_sales_fact_table = MySqlOperator(
                        ...
                        sql = "delete from sales_fact where date between '2021-07-25' and '2021-07-25'",
    )
    ...
    send_success_email = EmailOperator(
                task_id='send_email',
                to='shafiqul.islam@apsissoultions.com',
    ...
    # start_operator >> query_manager_table_to_sales_fact_table

    start_operator >> delete_rows_from_sales_fact_table

    delete_rows_from_sales_fact_table >> query_manager_table_to_sales_fact_table

    query_manager_table_to_sales_fact_table >> run_quality_checks
